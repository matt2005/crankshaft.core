...
MetricsEndpoint::MetricsEndpoint(QObject* parent)
    ...
    m_activeConnections = std::make_unique<MetricTimeSeries>("active_connections", "count", m_maxHistorySamples);
    m_totalConnections = std::make_unique<MetricTimeSeries>("total_connections", "count", m_maxHistorySamples);
    m_requestLatency = std::make_unique<MetricTimeSeries>("request_latency", "ms", m_maxHistorySamples);
...
void MetricsEndpoint::addAlert(const MetricAlert& alert) {
    ...
    qInfo() << "Added alert:" << alert.metricName << "warning:" << alert.warningThreshold << "critical:" << alert.criticalThreshold;
}
...
void MetricsEndpoint::evaluateAlert(const MetricAlert& alert, double currentValue) {
    if (currentValue >= alert.criticalThreshold) {
        ...
        qCritical() << "ALERT [CRITICAL]:" << alert.description
                    << "-" << alert.metricName << "=" << currentValue
        ...
        qWarning() << "ALERT [WARNING]:" << alert.description
                   << "-" << alert.metricName << "=" << currentValue
