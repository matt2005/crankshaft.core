...
class TestExtensionLifecycle : public QObject {
 ...
 private slots:
  ...
  void testInstallExtension() {
    ...
    QCOMPARE(ext.value(QStringLiteral("id")).toString(), QStringLiteral("test.extension.sample"));
  ...
  void testStartExtension() {
    ...
    QString result = m_extensionManager->startExtension(QStringLiteral("test.start.sample"));
    ...
    QJsonObject info = m_extensionManager->getExtensionInfo(QStringLiteral("test.start.sample"));
  ...
  void testStopExtension() {
    ...
    QString result = m_extensionManager->stopExtension(QStringLiteral("test.stop.sample"));
    ...
    QJsonObject info = m_extensionManager->getExtensionInfo(QStringLiteral("test.stop.sample"));
    QVERIFY(!info.value(QStringLiteral("is_running")).toBool());
  }
  ...
  void testRestartExtension() {
    ...
    QString result = m_extensionManager->restartExtension(QStringLiteral("test.restart.sample"));
  ...
  void testUninstallExtension() {
    ...
    QString result =
        m_extensionManager->uninstallExtension(QStringLiteral("test.uninstall.sample"));
  ...
  void testPermissionChecking() {
    ...
    // Test has permission
    QVERIFY(m_extensionManager->hasPermission(QStringLiteral("test.perms.sample"),
  ...
  void testGetExtensionsWithPermission() {
    ...
    for (int i = 0; i < 2; ++i) {
      QJsonObject manifest;
      manifest.insert(QStringLiteral("id"), QStringLiteral("test.filter.sample%1").arg(i));
      manifest.insert(QStringLiteral("name"), QStringLiteral("Test Filter %1").arg(i));
      manifest.insert(QStringLiteral("version"), QStringLiteral("1.0.0"));
      manifest.insert(QStringLiteral("entrypoint"), QStringLiteral("sleep"));
    ...
    QJsonArray withUiTile =
        m_extensionManager->getExtensionsWithPermission(QStringLiteral("ui.tile"));
    QCOMPARE(withUiTile.count(), 2);

    // Get all extensions with network permission
    QJsonArray withNetwork =
        m_extensionManager->getExtensionsWithPermission(QStringLiteral("network"));
  ...
  void testStartNonexistentExtension() {
    QString result = m_extensionManager->startExtension(QStringLiteral("nonexistent.extension"));
    QVERIFY(!result.isEmpty());
