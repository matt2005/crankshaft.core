# Changelog

This project follows a changelog generated via CI to keep track of notable changes. Latest entries are also published as a CI artifact (`changelog-ci.md`) from the **Changelog** workflow.

## Unreleased
- Add About page acknowledgements, SBOM link, and changelog link
- Add link and header setting components for richer settings UI
- Introduce multi-architecture build pipeline (amd64, arm64, armhf) with DEB packaging
- Generate CycloneDX SBOM as part of CI/CD
- Create dedicated changelog workflow and documentation entry
- Update maintainer contact to opencardevuk@gmail.com

## [2026.02.13+git.3d9eca1] - 2026-02-13
- 3d9eca1 Refactor build.sh to follow openauto pattern and fix issues (#35)
- b797f77 Docs: update CHANGELOG.md for 2026.02.12+git.96d28d5
- 96d28d5 Fix ARM cross-compilation compiler setup (#34)
- 9b2f1a3 Docs: update CHANGELOG.md for 2026.02.12+git.40a947c
- 40a947c Fix build artifacts export and add logging (#33)
- d5d2e8c Fix: Conditional protobuf installation based on BUILD_AASDK flag
- ac9e2be Fix: Use Alpine exporter stage for Docker build artifact extraction
- 4e1354b Docs: update CHANGELOG.md for 2026.01.31+git.57c1465
- 57c1465 Fix CI workflow: correct boolean input handling for reusable workflow call (#32)
- 2b112af fix(ci): clean workflow YAML and enable AASDK for SAST
- 2c9c371 fix(ci): enable AASDK for SAST and drop unsupported clang-tidy flag
- d98d647 Docs: update CHANGELOG.md for 2026.01.28+git.b72c718
- b72c718 feat: Implement BUILD_AASDK flag for flexible dependency management (#30)
- 00e7113 Docs: update CHANGELOG.md for 2026.01.21+git.0c6fcf9
- 0c6fcf9 Enable DEB packaging for ui-slim component (#28)
- f18c4ff Docs: update CHANGELOG.md for 2026.01.20+git.f052fde
- f052fde Fix: MOC compatibility and AA lifecycle test (#27)
- ad0b873 Docs: update CHANGELOG.md for 2026.01.20+git.e0e57a4
- e0e57a4 Fix: Restore slim UI RCC generation (#26)
- fec99bd Docs: update CHANGELOG.md for 2026.01.17+git.f71d777
- f71d777 Fix: Modernise return types and update aasdk (#25)
- 186d69c Docs: update CHANGELOG.md for 2026.01.12+git.f4b3143
- f4b3143 Fix: handle nodiscard return values from PreferencesService::set() in SettingsMigration (#24)
- 96855ac Docs: update CHANGELOG.md for 2026.01.12+git.4504e9e
- 4504e9e Slim AndroidAuto UI: add slim UI app, packaging, docs, and CI coverage (#23)
- 8c415f1 fix: comment out invalid wsConnections Connections property block
- 2598707 fix: remove stateChanged signal that conflicts with auto-generated state property signal
- a7afa71 fix: remove explicit displaysChanged signal, use auto-generated from displays property
- ad40d2b fix: remove duplicate Component.onCompleted and problematic Connections block
- 20a4c91 fix: defer SettingsModel access in AndroidAutoStatus to Component.onCompleted
- a7d0771 chore: add qmldir to .gitignore (auto-generated by Qt6 CMake)
- a8f3909 fix: remove conflicting primaryDisplayChanged signal, use auto-generated primaryDisplayIdChanged
- 15a5c06 fix: remove manual qmldir, let Qt 6 CMake auto-generate it
- a62b06a fix: add DisplayModel to qmldir as singleton
- 502e7e1 fix: add SettingsModel.cpp to CMakeLists and remove QML singleton marker
- d6477a3 fix: remove extra brace in SettingsModel.qml
- 27166fa fix: option D - create SettingsModel and SettingsRegistry as C++ context properties
- ad36b38 fix: add QML_ELEMENT to SettingsRegistry and delay SettingsModel init
- 9fe8305 fix: register SettingsRegistry as context property instead of QML singleton
- fa379c5 fix: register SettingsRegistry C++ singleton in qmldir
- 791ab51 fix: add SettingsRegistry include to UI main.cpp
- 65a7625 Docs: update CHANGELOG.md for 2026.01.10+git.fa44d28
- fa44d28 feat: Modern Responsive UI (AndroidAuto + Settings) (#22)
- 3a33b31 fix home
- 1f09776 Added prerm fix to apt
- 2b52e6b fix: Set default profile to use real devices (useMock=false)
- f38bcef fix: Create profiles directory in postinst
- 55719e3 format fix
- e626fd8 fix: Register AndroidAutoStatus as QML singleton in CMakeLists.txt
- e0ad389 Docs: update CHANGELOG.md for 2026.01.08+git.42b524b
- 42b524b fix: Normalise packaging scripts to LF and resolve QML singleton issues (#21)
- aaa4b6b Docs: update CHANGELOG.md for 2026.01.08+git.4158b14
- 4158b14 Bugfix/quality (#20)
- 5407ebe Docs: update CHANGELOG.md for 2026.01.07+git.2fc4b74
- 2fc4b74 format fix (#19)
- ed627bd Docs: update CHANGELOG.md for 2026.01.06+git.7800506
- 7800506 001 rpi image (#18)
- c74aa59 fix(image-builder): Make dpkg-reconfigure qemu-user-static non-interactive
- 3188f81 Docs: update CHANGELOG.md for 2026.01.05+git.3801f10
- 3801f10 fix(ci): Use trigger-apt-publish workflow instead of direct apt-publish (#17)
- 2445f76 Docs: update CHANGELOG.md for 2026.01.05+git.6408b90
- 6408b90 feat: Comprehensive GitHub Actions CI/CD Pipeline (Feature 003) (#16)
- f71dea3 Docs: update CHANGELOG.md for 2026.01.03+git.54c40bc
- 54c40bc feat: Crankshaft MVP - Phase 8 Complete (100% Project Delivery) (#15)
- 72d1dc0 Docs: update CHANGELOG.md for 2026.01.03+git.2dac628
- 2dac628 Docs: Add project constitution and enforce checks in templates (#14)
- 47e8b0d Docs: update CHANGELOG.md for 2025.12.30+git.3ed944a
- 3ed944a Added AA status widget (#13)
- 62899b9 Docs: update CHANGELOG.md for 2025.12.29+git.7bf85a9
- 7bf85a9 Fix: ProfileManager initialisation and core→UI state synchronisation (#12)
- 47d095e Docs: update CHANGELOG.md for 2025.12.21+git.0bc0d7c
- 0bc0d7c chore: Update aasdk submodule to main (feature/AA_part2) (#11)
- 734b794 Docs: update CHANGELOG.md for 2025.12.21+git.61f26ae
- 61f26ae Fix: Android Auto USB connection - wire USBHub device handle and poll io_service (#10)
- f139ccf Docs: update CHANGELOG.md for 2025.12.20+git.72246c8
- 72246c8 Feature: ProfileManager - schemas, validation, docs and build fixes (#9)
- 722b241 Docs: update CHANGELOG.md for 2025.12.19+git.5b9a14e
- 5b9a14e Fix: add qt6-connectivity-dev to Docker for Qt6 Bluetooth support (#8)
- 7421f07 Fix: remove ServiceManager stub causing vtable linkage errors (#7)
- dee56d6 Docs: update CHANGELOG.md for 2025.12.18+git.8ac2251
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2026.02.12+git.96d28d5] - 2026-02-12
- 96d28d5 Fix ARM cross-compilation compiler setup (#34)
- 9b2f1a3 Docs: update CHANGELOG.md for 2026.02.12+git.40a947c
- 40a947c Fix build artifacts export and add logging (#33)
- d5d2e8c Fix: Conditional protobuf installation based on BUILD_AASDK flag
- ac9e2be Fix: Use Alpine exporter stage for Docker build artifact extraction
- 4e1354b Docs: update CHANGELOG.md for 2026.01.31+git.57c1465
- 57c1465 Fix CI workflow: correct boolean input handling for reusable workflow call (#32)
- 2b112af fix(ci): clean workflow YAML and enable AASDK for SAST
- 2c9c371 fix(ci): enable AASDK for SAST and drop unsupported clang-tidy flag
- d98d647 Docs: update CHANGELOG.md for 2026.01.28+git.b72c718
- b72c718 feat: Implement BUILD_AASDK flag for flexible dependency management (#30)
- 00e7113 Docs: update CHANGELOG.md for 2026.01.21+git.0c6fcf9
- 0c6fcf9 Enable DEB packaging for ui-slim component (#28)
- f18c4ff Docs: update CHANGELOG.md for 2026.01.20+git.f052fde
- f052fde Fix: MOC compatibility and AA lifecycle test (#27)
- ad0b873 Docs: update CHANGELOG.md for 2026.01.20+git.e0e57a4
- e0e57a4 Fix: Restore slim UI RCC generation (#26)
- fec99bd Docs: update CHANGELOG.md for 2026.01.17+git.f71d777
- f71d777 Fix: Modernise return types and update aasdk (#25)
- 186d69c Docs: update CHANGELOG.md for 2026.01.12+git.f4b3143
- f4b3143 Fix: handle nodiscard return values from PreferencesService::set() in SettingsMigration (#24)
- 96855ac Docs: update CHANGELOG.md for 2026.01.12+git.4504e9e
- 4504e9e Slim AndroidAuto UI: add slim UI app, packaging, docs, and CI coverage (#23)
- 8c415f1 fix: comment out invalid wsConnections Connections property block
- 2598707 fix: remove stateChanged signal that conflicts with auto-generated state property signal
- a7afa71 fix: remove explicit displaysChanged signal, use auto-generated from displays property
- ad40d2b fix: remove duplicate Component.onCompleted and problematic Connections block
- 20a4c91 fix: defer SettingsModel access in AndroidAutoStatus to Component.onCompleted
- a7d0771 chore: add qmldir to .gitignore (auto-generated by Qt6 CMake)
- a8f3909 fix: remove conflicting primaryDisplayChanged signal, use auto-generated primaryDisplayIdChanged
- 15a5c06 fix: remove manual qmldir, let Qt 6 CMake auto-generate it
- a62b06a fix: add DisplayModel to qmldir as singleton
- 502e7e1 fix: add SettingsModel.cpp to CMakeLists and remove QML singleton marker
- d6477a3 fix: remove extra brace in SettingsModel.qml
- 27166fa fix: option D - create SettingsModel and SettingsRegistry as C++ context properties
- ad36b38 fix: add QML_ELEMENT to SettingsRegistry and delay SettingsModel init
- 9fe8305 fix: register SettingsRegistry as context property instead of QML singleton
- fa379c5 fix: register SettingsRegistry C++ singleton in qmldir
- 791ab51 fix: add SettingsRegistry include to UI main.cpp
- 65a7625 Docs: update CHANGELOG.md for 2026.01.10+git.fa44d28
- fa44d28 feat: Modern Responsive UI (AndroidAuto + Settings) (#22)
- 3a33b31 fix home
- 1f09776 Added prerm fix to apt
- 2b52e6b fix: Set default profile to use real devices (useMock=false)
- f38bcef fix: Create profiles directory in postinst
- 55719e3 format fix
- e626fd8 fix: Register AndroidAutoStatus as QML singleton in CMakeLists.txt
- e0ad389 Docs: update CHANGELOG.md for 2026.01.08+git.42b524b
- 42b524b fix: Normalise packaging scripts to LF and resolve QML singleton issues (#21)
- aaa4b6b Docs: update CHANGELOG.md for 2026.01.08+git.4158b14
- 4158b14 Bugfix/quality (#20)
- 5407ebe Docs: update CHANGELOG.md for 2026.01.07+git.2fc4b74
- 2fc4b74 format fix (#19)
- ed627bd Docs: update CHANGELOG.md for 2026.01.06+git.7800506
- 7800506 001 rpi image (#18)
- c74aa59 fix(image-builder): Make dpkg-reconfigure qemu-user-static non-interactive
- 3188f81 Docs: update CHANGELOG.md for 2026.01.05+git.3801f10
- 3801f10 fix(ci): Use trigger-apt-publish workflow instead of direct apt-publish (#17)
- 2445f76 Docs: update CHANGELOG.md for 2026.01.05+git.6408b90
- 6408b90 feat: Comprehensive GitHub Actions CI/CD Pipeline (Feature 003) (#16)
- f71dea3 Docs: update CHANGELOG.md for 2026.01.03+git.54c40bc
- 54c40bc feat: Crankshaft MVP - Phase 8 Complete (100% Project Delivery) (#15)
- 72d1dc0 Docs: update CHANGELOG.md for 2026.01.03+git.2dac628
- 2dac628 Docs: Add project constitution and enforce checks in templates (#14)
- 47e8b0d Docs: update CHANGELOG.md for 2025.12.30+git.3ed944a
- 3ed944a Added AA status widget (#13)
- 62899b9 Docs: update CHANGELOG.md for 2025.12.29+git.7bf85a9
- 7bf85a9 Fix: ProfileManager initialisation and core→UI state synchronisation (#12)
- 47d095e Docs: update CHANGELOG.md for 2025.12.21+git.0bc0d7c
- 0bc0d7c chore: Update aasdk submodule to main (feature/AA_part2) (#11)
- 734b794 Docs: update CHANGELOG.md for 2025.12.21+git.61f26ae
- 61f26ae Fix: Android Auto USB connection - wire USBHub device handle and poll io_service (#10)
- f139ccf Docs: update CHANGELOG.md for 2025.12.20+git.72246c8
- 72246c8 Feature: ProfileManager - schemas, validation, docs and build fixes (#9)
- 722b241 Docs: update CHANGELOG.md for 2025.12.19+git.5b9a14e
- 5b9a14e Fix: add qt6-connectivity-dev to Docker for Qt6 Bluetooth support (#8)
- 7421f07 Fix: remove ServiceManager stub causing vtable linkage errors (#7)
- dee56d6 Docs: update CHANGELOG.md for 2025.12.18+git.8ac2251
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2026.02.12+git.40a947c] - 2026-02-12
- 40a947c Fix build artifacts export and add logging (#33)
- d5d2e8c Fix: Conditional protobuf installation based on BUILD_AASDK flag
- ac9e2be Fix: Use Alpine exporter stage for Docker build artifact extraction
- 4e1354b Docs: update CHANGELOG.md for 2026.01.31+git.57c1465
- 57c1465 Fix CI workflow: correct boolean input handling for reusable workflow call (#32)
- 2b112af fix(ci): clean workflow YAML and enable AASDK for SAST
- 2c9c371 fix(ci): enable AASDK for SAST and drop unsupported clang-tidy flag
- d98d647 Docs: update CHANGELOG.md for 2026.01.28+git.b72c718
- b72c718 feat: Implement BUILD_AASDK flag for flexible dependency management (#30)
- 00e7113 Docs: update CHANGELOG.md for 2026.01.21+git.0c6fcf9
- 0c6fcf9 Enable DEB packaging for ui-slim component (#28)
- f18c4ff Docs: update CHANGELOG.md for 2026.01.20+git.f052fde
- f052fde Fix: MOC compatibility and AA lifecycle test (#27)
- ad0b873 Docs: update CHANGELOG.md for 2026.01.20+git.e0e57a4
- e0e57a4 Fix: Restore slim UI RCC generation (#26)
- fec99bd Docs: update CHANGELOG.md for 2026.01.17+git.f71d777
- f71d777 Fix: Modernise return types and update aasdk (#25)
- 186d69c Docs: update CHANGELOG.md for 2026.01.12+git.f4b3143
- f4b3143 Fix: handle nodiscard return values from PreferencesService::set() in SettingsMigration (#24)
- 96855ac Docs: update CHANGELOG.md for 2026.01.12+git.4504e9e
- 4504e9e Slim AndroidAuto UI: add slim UI app, packaging, docs, and CI coverage (#23)
- 8c415f1 fix: comment out invalid wsConnections Connections property block
- 2598707 fix: remove stateChanged signal that conflicts with auto-generated state property signal
- a7afa71 fix: remove explicit displaysChanged signal, use auto-generated from displays property
- ad40d2b fix: remove duplicate Component.onCompleted and problematic Connections block
- 20a4c91 fix: defer SettingsModel access in AndroidAutoStatus to Component.onCompleted
- a7d0771 chore: add qmldir to .gitignore (auto-generated by Qt6 CMake)
- a8f3909 fix: remove conflicting primaryDisplayChanged signal, use auto-generated primaryDisplayIdChanged
- 15a5c06 fix: remove manual qmldir, let Qt 6 CMake auto-generate it
- a62b06a fix: add DisplayModel to qmldir as singleton
- 502e7e1 fix: add SettingsModel.cpp to CMakeLists and remove QML singleton marker
- d6477a3 fix: remove extra brace in SettingsModel.qml
- 27166fa fix: option D - create SettingsModel and SettingsRegistry as C++ context properties
- ad36b38 fix: add QML_ELEMENT to SettingsRegistry and delay SettingsModel init
- 9fe8305 fix: register SettingsRegistry as context property instead of QML singleton
- fa379c5 fix: register SettingsRegistry C++ singleton in qmldir
- 791ab51 fix: add SettingsRegistry include to UI main.cpp
- 65a7625 Docs: update CHANGELOG.md for 2026.01.10+git.fa44d28
- fa44d28 feat: Modern Responsive UI (AndroidAuto + Settings) (#22)
- 3a33b31 fix home
- 1f09776 Added prerm fix to apt
- 2b52e6b fix: Set default profile to use real devices (useMock=false)
- f38bcef fix: Create profiles directory in postinst
- 55719e3 format fix
- e626fd8 fix: Register AndroidAutoStatus as QML singleton in CMakeLists.txt
- e0ad389 Docs: update CHANGELOG.md for 2026.01.08+git.42b524b
- 42b524b fix: Normalise packaging scripts to LF and resolve QML singleton issues (#21)
- aaa4b6b Docs: update CHANGELOG.md for 2026.01.08+git.4158b14
- 4158b14 Bugfix/quality (#20)
- 5407ebe Docs: update CHANGELOG.md for 2026.01.07+git.2fc4b74
- 2fc4b74 format fix (#19)
- ed627bd Docs: update CHANGELOG.md for 2026.01.06+git.7800506
- 7800506 001 rpi image (#18)
- c74aa59 fix(image-builder): Make dpkg-reconfigure qemu-user-static non-interactive
- 3188f81 Docs: update CHANGELOG.md for 2026.01.05+git.3801f10
- 3801f10 fix(ci): Use trigger-apt-publish workflow instead of direct apt-publish (#17)
- 2445f76 Docs: update CHANGELOG.md for 2026.01.05+git.6408b90
- 6408b90 feat: Comprehensive GitHub Actions CI/CD Pipeline (Feature 003) (#16)
- f71dea3 Docs: update CHANGELOG.md for 2026.01.03+git.54c40bc
- 54c40bc feat: Crankshaft MVP - Phase 8 Complete (100% Project Delivery) (#15)
- 72d1dc0 Docs: update CHANGELOG.md for 2026.01.03+git.2dac628
- 2dac628 Docs: Add project constitution and enforce checks in templates (#14)
- 47e8b0d Docs: update CHANGELOG.md for 2025.12.30+git.3ed944a
- 3ed944a Added AA status widget (#13)
- 62899b9 Docs: update CHANGELOG.md for 2025.12.29+git.7bf85a9
- 7bf85a9 Fix: ProfileManager initialisation and core→UI state synchronisation (#12)
- 47d095e Docs: update CHANGELOG.md for 2025.12.21+git.0bc0d7c
- 0bc0d7c chore: Update aasdk submodule to main (feature/AA_part2) (#11)
- 734b794 Docs: update CHANGELOG.md for 2025.12.21+git.61f26ae
- 61f26ae Fix: Android Auto USB connection - wire USBHub device handle and poll io_service (#10)
- f139ccf Docs: update CHANGELOG.md for 2025.12.20+git.72246c8
- 72246c8 Feature: ProfileManager - schemas, validation, docs and build fixes (#9)
- 722b241 Docs: update CHANGELOG.md for 2025.12.19+git.5b9a14e
- 5b9a14e Fix: add qt6-connectivity-dev to Docker for Qt6 Bluetooth support (#8)
- 7421f07 Fix: remove ServiceManager stub causing vtable linkage errors (#7)
- dee56d6 Docs: update CHANGELOG.md for 2025.12.18+git.8ac2251
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2026.01.31+git.57c1465] - 2026-01-31
- 57c1465 Fix CI workflow: correct boolean input handling for reusable workflow call (#32)
- 2b112af fix(ci): clean workflow YAML and enable AASDK for SAST
- 2c9c371 fix(ci): enable AASDK for SAST and drop unsupported clang-tidy flag
- d98d647 Docs: update CHANGELOG.md for 2026.01.28+git.b72c718
- b72c718 feat: Implement BUILD_AASDK flag for flexible dependency management (#30)
- 00e7113 Docs: update CHANGELOG.md for 2026.01.21+git.0c6fcf9
- 0c6fcf9 Enable DEB packaging for ui-slim component (#28)
- f18c4ff Docs: update CHANGELOG.md for 2026.01.20+git.f052fde
- f052fde Fix: MOC compatibility and AA lifecycle test (#27)
- ad0b873 Docs: update CHANGELOG.md for 2026.01.20+git.e0e57a4
- e0e57a4 Fix: Restore slim UI RCC generation (#26)
- fec99bd Docs: update CHANGELOG.md for 2026.01.17+git.f71d777
- f71d777 Fix: Modernise return types and update aasdk (#25)
- 186d69c Docs: update CHANGELOG.md for 2026.01.12+git.f4b3143
- f4b3143 Fix: handle nodiscard return values from PreferencesService::set() in SettingsMigration (#24)
- 96855ac Docs: update CHANGELOG.md for 2026.01.12+git.4504e9e
- 4504e9e Slim AndroidAuto UI: add slim UI app, packaging, docs, and CI coverage (#23)
- 8c415f1 fix: comment out invalid wsConnections Connections property block
- 2598707 fix: remove stateChanged signal that conflicts with auto-generated state property signal
- a7afa71 fix: remove explicit displaysChanged signal, use auto-generated from displays property
- ad40d2b fix: remove duplicate Component.onCompleted and problematic Connections block
- 20a4c91 fix: defer SettingsModel access in AndroidAutoStatus to Component.onCompleted
- a7d0771 chore: add qmldir to .gitignore (auto-generated by Qt6 CMake)
- a8f3909 fix: remove conflicting primaryDisplayChanged signal, use auto-generated primaryDisplayIdChanged
- 15a5c06 fix: remove manual qmldir, let Qt 6 CMake auto-generate it
- a62b06a fix: add DisplayModel to qmldir as singleton
- 502e7e1 fix: add SettingsModel.cpp to CMakeLists and remove QML singleton marker
- d6477a3 fix: remove extra brace in SettingsModel.qml
- 27166fa fix: option D - create SettingsModel and SettingsRegistry as C++ context properties
- ad36b38 fix: add QML_ELEMENT to SettingsRegistry and delay SettingsModel init
- 9fe8305 fix: register SettingsRegistry as context property instead of QML singleton
- fa379c5 fix: register SettingsRegistry C++ singleton in qmldir
- 791ab51 fix: add SettingsRegistry include to UI main.cpp
- 65a7625 Docs: update CHANGELOG.md for 2026.01.10+git.fa44d28
- fa44d28 feat: Modern Responsive UI (AndroidAuto + Settings) (#22)
- 3a33b31 fix home
- 1f09776 Added prerm fix to apt
- 2b52e6b fix: Set default profile to use real devices (useMock=false)
- f38bcef fix: Create profiles directory in postinst
- 55719e3 format fix
- e626fd8 fix: Register AndroidAutoStatus as QML singleton in CMakeLists.txt
- e0ad389 Docs: update CHANGELOG.md for 2026.01.08+git.42b524b
- 42b524b fix: Normalise packaging scripts to LF and resolve QML singleton issues (#21)
- aaa4b6b Docs: update CHANGELOG.md for 2026.01.08+git.4158b14
- 4158b14 Bugfix/quality (#20)
- 5407ebe Docs: update CHANGELOG.md for 2026.01.07+git.2fc4b74
- 2fc4b74 format fix (#19)
- ed627bd Docs: update CHANGELOG.md for 2026.01.06+git.7800506
- 7800506 001 rpi image (#18)
- c74aa59 fix(image-builder): Make dpkg-reconfigure qemu-user-static non-interactive
- 3188f81 Docs: update CHANGELOG.md for 2026.01.05+git.3801f10
- 3801f10 fix(ci): Use trigger-apt-publish workflow instead of direct apt-publish (#17)
- 2445f76 Docs: update CHANGELOG.md for 2026.01.05+git.6408b90
- 6408b90 feat: Comprehensive GitHub Actions CI/CD Pipeline (Feature 003) (#16)
- f71dea3 Docs: update CHANGELOG.md for 2026.01.03+git.54c40bc
- 54c40bc feat: Crankshaft MVP - Phase 8 Complete (100% Project Delivery) (#15)
- 72d1dc0 Docs: update CHANGELOG.md for 2026.01.03+git.2dac628
- 2dac628 Docs: Add project constitution and enforce checks in templates (#14)
- 47e8b0d Docs: update CHANGELOG.md for 2025.12.30+git.3ed944a
- 3ed944a Added AA status widget (#13)
- 62899b9 Docs: update CHANGELOG.md for 2025.12.29+git.7bf85a9
- 7bf85a9 Fix: ProfileManager initialisation and core→UI state synchronisation (#12)
- 47d095e Docs: update CHANGELOG.md for 2025.12.21+git.0bc0d7c
- 0bc0d7c chore: Update aasdk submodule to main (feature/AA_part2) (#11)
- 734b794 Docs: update CHANGELOG.md for 2025.12.21+git.61f26ae
- 61f26ae Fix: Android Auto USB connection - wire USBHub device handle and poll io_service (#10)
- f139ccf Docs: update CHANGELOG.md for 2025.12.20+git.72246c8
- 72246c8 Feature: ProfileManager - schemas, validation, docs and build fixes (#9)
- 722b241 Docs: update CHANGELOG.md for 2025.12.19+git.5b9a14e
- 5b9a14e Fix: add qt6-connectivity-dev to Docker for Qt6 Bluetooth support (#8)
- 7421f07 Fix: remove ServiceManager stub causing vtable linkage errors (#7)
- dee56d6 Docs: update CHANGELOG.md for 2025.12.18+git.8ac2251
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2026.01.28+git.b72c718] - 2026-01-28
- b72c718 feat: Implement BUILD_AASDK flag for flexible dependency management (#30)
- 00e7113 Docs: update CHANGELOG.md for 2026.01.21+git.0c6fcf9
- 0c6fcf9 Enable DEB packaging for ui-slim component (#28)
- f18c4ff Docs: update CHANGELOG.md for 2026.01.20+git.f052fde
- f052fde Fix: MOC compatibility and AA lifecycle test (#27)
- ad0b873 Docs: update CHANGELOG.md for 2026.01.20+git.e0e57a4
- e0e57a4 Fix: Restore slim UI RCC generation (#26)
- fec99bd Docs: update CHANGELOG.md for 2026.01.17+git.f71d777
- f71d777 Fix: Modernise return types and update aasdk (#25)
- 186d69c Docs: update CHANGELOG.md for 2026.01.12+git.f4b3143
- f4b3143 Fix: handle nodiscard return values from PreferencesService::set() in SettingsMigration (#24)
- 96855ac Docs: update CHANGELOG.md for 2026.01.12+git.4504e9e
- 4504e9e Slim AndroidAuto UI: add slim UI app, packaging, docs, and CI coverage (#23)
- 8c415f1 fix: comment out invalid wsConnections Connections property block
- 2598707 fix: remove stateChanged signal that conflicts with auto-generated state property signal
- a7afa71 fix: remove explicit displaysChanged signal, use auto-generated from displays property
- ad40d2b fix: remove duplicate Component.onCompleted and problematic Connections block
- 20a4c91 fix: defer SettingsModel access in AndroidAutoStatus to Component.onCompleted
- a7d0771 chore: add qmldir to .gitignore (auto-generated by Qt6 CMake)
- a8f3909 fix: remove conflicting primaryDisplayChanged signal, use auto-generated primaryDisplayIdChanged
- 15a5c06 fix: remove manual qmldir, let Qt 6 CMake auto-generate it
- a62b06a fix: add DisplayModel to qmldir as singleton
- 502e7e1 fix: add SettingsModel.cpp to CMakeLists and remove QML singleton marker
- d6477a3 fix: remove extra brace in SettingsModel.qml
- 27166fa fix: option D - create SettingsModel and SettingsRegistry as C++ context properties
- ad36b38 fix: add QML_ELEMENT to SettingsRegistry and delay SettingsModel init
- 9fe8305 fix: register SettingsRegistry as context property instead of QML singleton
- fa379c5 fix: register SettingsRegistry C++ singleton in qmldir
- 791ab51 fix: add SettingsRegistry include to UI main.cpp
- 65a7625 Docs: update CHANGELOG.md for 2026.01.10+git.fa44d28
- fa44d28 feat: Modern Responsive UI (AndroidAuto + Settings) (#22)
- 3a33b31 fix home
- 1f09776 Added prerm fix to apt
- 2b52e6b fix: Set default profile to use real devices (useMock=false)
- f38bcef fix: Create profiles directory in postinst
- 55719e3 format fix
- e626fd8 fix: Register AndroidAutoStatus as QML singleton in CMakeLists.txt
- e0ad389 Docs: update CHANGELOG.md for 2026.01.08+git.42b524b
- 42b524b fix: Normalise packaging scripts to LF and resolve QML singleton issues (#21)
- aaa4b6b Docs: update CHANGELOG.md for 2026.01.08+git.4158b14
- 4158b14 Bugfix/quality (#20)
- 5407ebe Docs: update CHANGELOG.md for 2026.01.07+git.2fc4b74
- 2fc4b74 format fix (#19)
- ed627bd Docs: update CHANGELOG.md for 2026.01.06+git.7800506
- 7800506 001 rpi image (#18)
- c74aa59 fix(image-builder): Make dpkg-reconfigure qemu-user-static non-interactive
- 3188f81 Docs: update CHANGELOG.md for 2026.01.05+git.3801f10
- 3801f10 fix(ci): Use trigger-apt-publish workflow instead of direct apt-publish (#17)
- 2445f76 Docs: update CHANGELOG.md for 2026.01.05+git.6408b90
- 6408b90 feat: Comprehensive GitHub Actions CI/CD Pipeline (Feature 003) (#16)
- f71dea3 Docs: update CHANGELOG.md for 2026.01.03+git.54c40bc
- 54c40bc feat: Crankshaft MVP - Phase 8 Complete (100% Project Delivery) (#15)
- 72d1dc0 Docs: update CHANGELOG.md for 2026.01.03+git.2dac628
- 2dac628 Docs: Add project constitution and enforce checks in templates (#14)
- 47e8b0d Docs: update CHANGELOG.md for 2025.12.30+git.3ed944a
- 3ed944a Added AA status widget (#13)
- 62899b9 Docs: update CHANGELOG.md for 2025.12.29+git.7bf85a9
- 7bf85a9 Fix: ProfileManager initialisation and core→UI state synchronisation (#12)
- 47d095e Docs: update CHANGELOG.md for 2025.12.21+git.0bc0d7c
- 0bc0d7c chore: Update aasdk submodule to main (feature/AA_part2) (#11)
- 734b794 Docs: update CHANGELOG.md for 2025.12.21+git.61f26ae
- 61f26ae Fix: Android Auto USB connection - wire USBHub device handle and poll io_service (#10)
- f139ccf Docs: update CHANGELOG.md for 2025.12.20+git.72246c8
- 72246c8 Feature: ProfileManager - schemas, validation, docs and build fixes (#9)
- 722b241 Docs: update CHANGELOG.md for 2025.12.19+git.5b9a14e
- 5b9a14e Fix: add qt6-connectivity-dev to Docker for Qt6 Bluetooth support (#8)
- 7421f07 Fix: remove ServiceManager stub causing vtable linkage errors (#7)
- dee56d6 Docs: update CHANGELOG.md for 2025.12.18+git.8ac2251
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2026.01.21+git.0c6fcf9] - 2026-01-21
- 0c6fcf9 Enable DEB packaging for ui-slim component (#28)
- f18c4ff Docs: update CHANGELOG.md for 2026.01.20+git.f052fde
- f052fde Fix: MOC compatibility and AA lifecycle test (#27)
- ad0b873 Docs: update CHANGELOG.md for 2026.01.20+git.e0e57a4
- e0e57a4 Fix: Restore slim UI RCC generation (#26)
- fec99bd Docs: update CHANGELOG.md for 2026.01.17+git.f71d777
- f71d777 Fix: Modernise return types and update aasdk (#25)
- 186d69c Docs: update CHANGELOG.md for 2026.01.12+git.f4b3143
- f4b3143 Fix: handle nodiscard return values from PreferencesService::set() in SettingsMigration (#24)
- 96855ac Docs: update CHANGELOG.md for 2026.01.12+git.4504e9e
- 4504e9e Slim AndroidAuto UI: add slim UI app, packaging, docs, and CI coverage (#23)
- 8c415f1 fix: comment out invalid wsConnections Connections property block
- 2598707 fix: remove stateChanged signal that conflicts with auto-generated state property signal
- a7afa71 fix: remove explicit displaysChanged signal, use auto-generated from displays property
- ad40d2b fix: remove duplicate Component.onCompleted and problematic Connections block
- 20a4c91 fix: defer SettingsModel access in AndroidAutoStatus to Component.onCompleted
- a7d0771 chore: add qmldir to .gitignore (auto-generated by Qt6 CMake)
- a8f3909 fix: remove conflicting primaryDisplayChanged signal, use auto-generated primaryDisplayIdChanged
- 15a5c06 fix: remove manual qmldir, let Qt 6 CMake auto-generate it
- a62b06a fix: add DisplayModel to qmldir as singleton
- 502e7e1 fix: add SettingsModel.cpp to CMakeLists and remove QML singleton marker
- d6477a3 fix: remove extra brace in SettingsModel.qml
- 27166fa fix: option D - create SettingsModel and SettingsRegistry as C++ context properties
- ad36b38 fix: add QML_ELEMENT to SettingsRegistry and delay SettingsModel init
- 9fe8305 fix: register SettingsRegistry as context property instead of QML singleton
- fa379c5 fix: register SettingsRegistry C++ singleton in qmldir
- 791ab51 fix: add SettingsRegistry include to UI main.cpp
- 65a7625 Docs: update CHANGELOG.md for 2026.01.10+git.fa44d28
- fa44d28 feat: Modern Responsive UI (AndroidAuto + Settings) (#22)
- 3a33b31 fix home
- 1f09776 Added prerm fix to apt
- 2b52e6b fix: Set default profile to use real devices (useMock=false)
- f38bcef fix: Create profiles directory in postinst
- 55719e3 format fix
- e626fd8 fix: Register AndroidAutoStatus as QML singleton in CMakeLists.txt
- e0ad389 Docs: update CHANGELOG.md for 2026.01.08+git.42b524b
- 42b524b fix: Normalise packaging scripts to LF and resolve QML singleton issues (#21)
- aaa4b6b Docs: update CHANGELOG.md for 2026.01.08+git.4158b14
- 4158b14 Bugfix/quality (#20)
- 5407ebe Docs: update CHANGELOG.md for 2026.01.07+git.2fc4b74
- 2fc4b74 format fix (#19)
- ed627bd Docs: update CHANGELOG.md for 2026.01.06+git.7800506
- 7800506 001 rpi image (#18)
- c74aa59 fix(image-builder): Make dpkg-reconfigure qemu-user-static non-interactive
- 3188f81 Docs: update CHANGELOG.md for 2026.01.05+git.3801f10
- 3801f10 fix(ci): Use trigger-apt-publish workflow instead of direct apt-publish (#17)
- 2445f76 Docs: update CHANGELOG.md for 2026.01.05+git.6408b90
- 6408b90 feat: Comprehensive GitHub Actions CI/CD Pipeline (Feature 003) (#16)
- f71dea3 Docs: update CHANGELOG.md for 2026.01.03+git.54c40bc
- 54c40bc feat: Crankshaft MVP - Phase 8 Complete (100% Project Delivery) (#15)
- 72d1dc0 Docs: update CHANGELOG.md for 2026.01.03+git.2dac628
- 2dac628 Docs: Add project constitution and enforce checks in templates (#14)
- 47e8b0d Docs: update CHANGELOG.md for 2025.12.30+git.3ed944a
- 3ed944a Added AA status widget (#13)
- 62899b9 Docs: update CHANGELOG.md for 2025.12.29+git.7bf85a9
- 7bf85a9 Fix: ProfileManager initialisation and core→UI state synchronisation (#12)
- 47d095e Docs: update CHANGELOG.md for 2025.12.21+git.0bc0d7c
- 0bc0d7c chore: Update aasdk submodule to main (feature/AA_part2) (#11)
- 734b794 Docs: update CHANGELOG.md for 2025.12.21+git.61f26ae
- 61f26ae Fix: Android Auto USB connection - wire USBHub device handle and poll io_service (#10)
- f139ccf Docs: update CHANGELOG.md for 2025.12.20+git.72246c8
- 72246c8 Feature: ProfileManager - schemas, validation, docs and build fixes (#9)
- 722b241 Docs: update CHANGELOG.md for 2025.12.19+git.5b9a14e
- 5b9a14e Fix: add qt6-connectivity-dev to Docker for Qt6 Bluetooth support (#8)
- 7421f07 Fix: remove ServiceManager stub causing vtable linkage errors (#7)
- dee56d6 Docs: update CHANGELOG.md for 2025.12.18+git.8ac2251
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2026.01.20+git.f052fde] - 2026-01-20
- f052fde Fix: MOC compatibility and AA lifecycle test (#27)
- ad0b873 Docs: update CHANGELOG.md for 2026.01.20+git.e0e57a4
- e0e57a4 Fix: Restore slim UI RCC generation (#26)
- fec99bd Docs: update CHANGELOG.md for 2026.01.17+git.f71d777
- f71d777 Fix: Modernise return types and update aasdk (#25)
- 186d69c Docs: update CHANGELOG.md for 2026.01.12+git.f4b3143
- f4b3143 Fix: handle nodiscard return values from PreferencesService::set() in SettingsMigration (#24)
- 96855ac Docs: update CHANGELOG.md for 2026.01.12+git.4504e9e
- 4504e9e Slim AndroidAuto UI: add slim UI app, packaging, docs, and CI coverage (#23)
- 8c415f1 fix: comment out invalid wsConnections Connections property block
- 2598707 fix: remove stateChanged signal that conflicts with auto-generated state property signal
- a7afa71 fix: remove explicit displaysChanged signal, use auto-generated from displays property
- ad40d2b fix: remove duplicate Component.onCompleted and problematic Connections block
- 20a4c91 fix: defer SettingsModel access in AndroidAutoStatus to Component.onCompleted
- a7d0771 chore: add qmldir to .gitignore (auto-generated by Qt6 CMake)
- a8f3909 fix: remove conflicting primaryDisplayChanged signal, use auto-generated primaryDisplayIdChanged
- 15a5c06 fix: remove manual qmldir, let Qt 6 CMake auto-generate it
- a62b06a fix: add DisplayModel to qmldir as singleton
- 502e7e1 fix: add SettingsModel.cpp to CMakeLists and remove QML singleton marker
- d6477a3 fix: remove extra brace in SettingsModel.qml
- 27166fa fix: option D - create SettingsModel and SettingsRegistry as C++ context properties
- ad36b38 fix: add QML_ELEMENT to SettingsRegistry and delay SettingsModel init
- 9fe8305 fix: register SettingsRegistry as context property instead of QML singleton
- fa379c5 fix: register SettingsRegistry C++ singleton in qmldir
- 791ab51 fix: add SettingsRegistry include to UI main.cpp
- 65a7625 Docs: update CHANGELOG.md for 2026.01.10+git.fa44d28
- fa44d28 feat: Modern Responsive UI (AndroidAuto + Settings) (#22)
- 3a33b31 fix home
- 1f09776 Added prerm fix to apt
- 2b52e6b fix: Set default profile to use real devices (useMock=false)
- f38bcef fix: Create profiles directory in postinst
- 55719e3 format fix
- e626fd8 fix: Register AndroidAutoStatus as QML singleton in CMakeLists.txt
- e0ad389 Docs: update CHANGELOG.md for 2026.01.08+git.42b524b
- 42b524b fix: Normalise packaging scripts to LF and resolve QML singleton issues (#21)
- aaa4b6b Docs: update CHANGELOG.md for 2026.01.08+git.4158b14
- 4158b14 Bugfix/quality (#20)
- 5407ebe Docs: update CHANGELOG.md for 2026.01.07+git.2fc4b74
- 2fc4b74 format fix (#19)
- ed627bd Docs: update CHANGELOG.md for 2026.01.06+git.7800506
- 7800506 001 rpi image (#18)
- c74aa59 fix(image-builder): Make dpkg-reconfigure qemu-user-static non-interactive
- 3188f81 Docs: update CHANGELOG.md for 2026.01.05+git.3801f10
- 3801f10 fix(ci): Use trigger-apt-publish workflow instead of direct apt-publish (#17)
- 2445f76 Docs: update CHANGELOG.md for 2026.01.05+git.6408b90
- 6408b90 feat: Comprehensive GitHub Actions CI/CD Pipeline (Feature 003) (#16)
- f71dea3 Docs: update CHANGELOG.md for 2026.01.03+git.54c40bc
- 54c40bc feat: Crankshaft MVP - Phase 8 Complete (100% Project Delivery) (#15)
- 72d1dc0 Docs: update CHANGELOG.md for 2026.01.03+git.2dac628
- 2dac628 Docs: Add project constitution and enforce checks in templates (#14)
- 47e8b0d Docs: update CHANGELOG.md for 2025.12.30+git.3ed944a
- 3ed944a Added AA status widget (#13)
- 62899b9 Docs: update CHANGELOG.md for 2025.12.29+git.7bf85a9
- 7bf85a9 Fix: ProfileManager initialisation and core→UI state synchronisation (#12)
- 47d095e Docs: update CHANGELOG.md for 2025.12.21+git.0bc0d7c
- 0bc0d7c chore: Update aasdk submodule to main (feature/AA_part2) (#11)
- 734b794 Docs: update CHANGELOG.md for 2025.12.21+git.61f26ae
- 61f26ae Fix: Android Auto USB connection - wire USBHub device handle and poll io_service (#10)
- f139ccf Docs: update CHANGELOG.md for 2025.12.20+git.72246c8
- 72246c8 Feature: ProfileManager - schemas, validation, docs and build fixes (#9)
- 722b241 Docs: update CHANGELOG.md for 2025.12.19+git.5b9a14e
- 5b9a14e Fix: add qt6-connectivity-dev to Docker for Qt6 Bluetooth support (#8)
- 7421f07 Fix: remove ServiceManager stub causing vtable linkage errors (#7)
- dee56d6 Docs: update CHANGELOG.md for 2025.12.18+git.8ac2251
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2026.01.20+git.e0e57a4] - 2026-01-20
- e0e57a4 Fix: Restore slim UI RCC generation (#26)
- fec99bd Docs: update CHANGELOG.md for 2026.01.17+git.f71d777
- f71d777 Fix: Modernise return types and update aasdk (#25)
- 186d69c Docs: update CHANGELOG.md for 2026.01.12+git.f4b3143
- f4b3143 Fix: handle nodiscard return values from PreferencesService::set() in SettingsMigration (#24)
- 96855ac Docs: update CHANGELOG.md for 2026.01.12+git.4504e9e
- 4504e9e Slim AndroidAuto UI: add slim UI app, packaging, docs, and CI coverage (#23)
- 8c415f1 fix: comment out invalid wsConnections Connections property block
- 2598707 fix: remove stateChanged signal that conflicts with auto-generated state property signal
- a7afa71 fix: remove explicit displaysChanged signal, use auto-generated from displays property
- ad40d2b fix: remove duplicate Component.onCompleted and problematic Connections block
- 20a4c91 fix: defer SettingsModel access in AndroidAutoStatus to Component.onCompleted
- a7d0771 chore: add qmldir to .gitignore (auto-generated by Qt6 CMake)
- a8f3909 fix: remove conflicting primaryDisplayChanged signal, use auto-generated primaryDisplayIdChanged
- 15a5c06 fix: remove manual qmldir, let Qt 6 CMake auto-generate it
- a62b06a fix: add DisplayModel to qmldir as singleton
- 502e7e1 fix: add SettingsModel.cpp to CMakeLists and remove QML singleton marker
- d6477a3 fix: remove extra brace in SettingsModel.qml
- 27166fa fix: option D - create SettingsModel and SettingsRegistry as C++ context properties
- ad36b38 fix: add QML_ELEMENT to SettingsRegistry and delay SettingsModel init
- 9fe8305 fix: register SettingsRegistry as context property instead of QML singleton
- fa379c5 fix: register SettingsRegistry C++ singleton in qmldir
- 791ab51 fix: add SettingsRegistry include to UI main.cpp
- 65a7625 Docs: update CHANGELOG.md for 2026.01.10+git.fa44d28
- fa44d28 feat: Modern Responsive UI (AndroidAuto + Settings) (#22)
- 3a33b31 fix home
- 1f09776 Added prerm fix to apt
- 2b52e6b fix: Set default profile to use real devices (useMock=false)
- f38bcef fix: Create profiles directory in postinst
- 55719e3 format fix
- e626fd8 fix: Register AndroidAutoStatus as QML singleton in CMakeLists.txt
- e0ad389 Docs: update CHANGELOG.md for 2026.01.08+git.42b524b
- 42b524b fix: Normalise packaging scripts to LF and resolve QML singleton issues (#21)
- aaa4b6b Docs: update CHANGELOG.md for 2026.01.08+git.4158b14
- 4158b14 Bugfix/quality (#20)
- 5407ebe Docs: update CHANGELOG.md for 2026.01.07+git.2fc4b74
- 2fc4b74 format fix (#19)
- ed627bd Docs: update CHANGELOG.md for 2026.01.06+git.7800506
- 7800506 001 rpi image (#18)
- c74aa59 fix(image-builder): Make dpkg-reconfigure qemu-user-static non-interactive
- 3188f81 Docs: update CHANGELOG.md for 2026.01.05+git.3801f10
- 3801f10 fix(ci): Use trigger-apt-publish workflow instead of direct apt-publish (#17)
- 2445f76 Docs: update CHANGELOG.md for 2026.01.05+git.6408b90
- 6408b90 feat: Comprehensive GitHub Actions CI/CD Pipeline (Feature 003) (#16)
- f71dea3 Docs: update CHANGELOG.md for 2026.01.03+git.54c40bc
- 54c40bc feat: Crankshaft MVP - Phase 8 Complete (100% Project Delivery) (#15)
- 72d1dc0 Docs: update CHANGELOG.md for 2026.01.03+git.2dac628
- 2dac628 Docs: Add project constitution and enforce checks in templates (#14)
- 47e8b0d Docs: update CHANGELOG.md for 2025.12.30+git.3ed944a
- 3ed944a Added AA status widget (#13)
- 62899b9 Docs: update CHANGELOG.md for 2025.12.29+git.7bf85a9
- 7bf85a9 Fix: ProfileManager initialisation and core→UI state synchronisation (#12)
- 47d095e Docs: update CHANGELOG.md for 2025.12.21+git.0bc0d7c
- 0bc0d7c chore: Update aasdk submodule to main (feature/AA_part2) (#11)
- 734b794 Docs: update CHANGELOG.md for 2025.12.21+git.61f26ae
- 61f26ae Fix: Android Auto USB connection - wire USBHub device handle and poll io_service (#10)
- f139ccf Docs: update CHANGELOG.md for 2025.12.20+git.72246c8
- 72246c8 Feature: ProfileManager - schemas, validation, docs and build fixes (#9)
- 722b241 Docs: update CHANGELOG.md for 2025.12.19+git.5b9a14e
- 5b9a14e Fix: add qt6-connectivity-dev to Docker for Qt6 Bluetooth support (#8)
- 7421f07 Fix: remove ServiceManager stub causing vtable linkage errors (#7)
- dee56d6 Docs: update CHANGELOG.md for 2025.12.18+git.8ac2251
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2026.01.17+git.f71d777] - 2026-01-17
- f71d777 Fix: Modernise return types and update aasdk (#25)
- 186d69c Docs: update CHANGELOG.md for 2026.01.12+git.f4b3143
- f4b3143 Fix: handle nodiscard return values from PreferencesService::set() in SettingsMigration (#24)
- 96855ac Docs: update CHANGELOG.md for 2026.01.12+git.4504e9e
- 4504e9e Slim AndroidAuto UI: add slim UI app, packaging, docs, and CI coverage (#23)
- 8c415f1 fix: comment out invalid wsConnections Connections property block
- 2598707 fix: remove stateChanged signal that conflicts with auto-generated state property signal
- a7afa71 fix: remove explicit displaysChanged signal, use auto-generated from displays property
- ad40d2b fix: remove duplicate Component.onCompleted and problematic Connections block
- 20a4c91 fix: defer SettingsModel access in AndroidAutoStatus to Component.onCompleted
- a7d0771 chore: add qmldir to .gitignore (auto-generated by Qt6 CMake)
- a8f3909 fix: remove conflicting primaryDisplayChanged signal, use auto-generated primaryDisplayIdChanged
- 15a5c06 fix: remove manual qmldir, let Qt 6 CMake auto-generate it
- a62b06a fix: add DisplayModel to qmldir as singleton
- 502e7e1 fix: add SettingsModel.cpp to CMakeLists and remove QML singleton marker
- d6477a3 fix: remove extra brace in SettingsModel.qml
- 27166fa fix: option D - create SettingsModel and SettingsRegistry as C++ context properties
- ad36b38 fix: add QML_ELEMENT to SettingsRegistry and delay SettingsModel init
- 9fe8305 fix: register SettingsRegistry as context property instead of QML singleton
- fa379c5 fix: register SettingsRegistry C++ singleton in qmldir
- 791ab51 fix: add SettingsRegistry include to UI main.cpp
- 65a7625 Docs: update CHANGELOG.md for 2026.01.10+git.fa44d28
- fa44d28 feat: Modern Responsive UI (AndroidAuto + Settings) (#22)
- 3a33b31 fix home
- 1f09776 Added prerm fix to apt
- 2b52e6b fix: Set default profile to use real devices (useMock=false)
- f38bcef fix: Create profiles directory in postinst
- 55719e3 format fix
- e626fd8 fix: Register AndroidAutoStatus as QML singleton in CMakeLists.txt
- e0ad389 Docs: update CHANGELOG.md for 2026.01.08+git.42b524b
- 42b524b fix: Normalise packaging scripts to LF and resolve QML singleton issues (#21)
- aaa4b6b Docs: update CHANGELOG.md for 2026.01.08+git.4158b14
- 4158b14 Bugfix/quality (#20)
- 5407ebe Docs: update CHANGELOG.md for 2026.01.07+git.2fc4b74
- 2fc4b74 format fix (#19)
- ed627bd Docs: update CHANGELOG.md for 2026.01.06+git.7800506
- 7800506 001 rpi image (#18)
- c74aa59 fix(image-builder): Make dpkg-reconfigure qemu-user-static non-interactive
- 3188f81 Docs: update CHANGELOG.md for 2026.01.05+git.3801f10
- 3801f10 fix(ci): Use trigger-apt-publish workflow instead of direct apt-publish (#17)
- 2445f76 Docs: update CHANGELOG.md for 2026.01.05+git.6408b90
- 6408b90 feat: Comprehensive GitHub Actions CI/CD Pipeline (Feature 003) (#16)
- f71dea3 Docs: update CHANGELOG.md for 2026.01.03+git.54c40bc
- 54c40bc feat: Crankshaft MVP - Phase 8 Complete (100% Project Delivery) (#15)
- 72d1dc0 Docs: update CHANGELOG.md for 2026.01.03+git.2dac628
- 2dac628 Docs: Add project constitution and enforce checks in templates (#14)
- 47e8b0d Docs: update CHANGELOG.md for 2025.12.30+git.3ed944a
- 3ed944a Added AA status widget (#13)
- 62899b9 Docs: update CHANGELOG.md for 2025.12.29+git.7bf85a9
- 7bf85a9 Fix: ProfileManager initialisation and core→UI state synchronisation (#12)
- 47d095e Docs: update CHANGELOG.md for 2025.12.21+git.0bc0d7c
- 0bc0d7c chore: Update aasdk submodule to main (feature/AA_part2) (#11)
- 734b794 Docs: update CHANGELOG.md for 2025.12.21+git.61f26ae
- 61f26ae Fix: Android Auto USB connection - wire USBHub device handle and poll io_service (#10)
- f139ccf Docs: update CHANGELOG.md for 2025.12.20+git.72246c8
- 72246c8 Feature: ProfileManager - schemas, validation, docs and build fixes (#9)
- 722b241 Docs: update CHANGELOG.md for 2025.12.19+git.5b9a14e
- 5b9a14e Fix: add qt6-connectivity-dev to Docker for Qt6 Bluetooth support (#8)
- 7421f07 Fix: remove ServiceManager stub causing vtable linkage errors (#7)
- dee56d6 Docs: update CHANGELOG.md for 2025.12.18+git.8ac2251
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2026.01.12+git.f4b3143] - 2026-01-12
- f4b3143 Fix: handle nodiscard return values from PreferencesService::set() in SettingsMigration (#24)
- 96855ac Docs: update CHANGELOG.md for 2026.01.12+git.4504e9e
- 4504e9e Slim AndroidAuto UI: add slim UI app, packaging, docs, and CI coverage (#23)
- 8c415f1 fix: comment out invalid wsConnections Connections property block
- 2598707 fix: remove stateChanged signal that conflicts with auto-generated state property signal
- a7afa71 fix: remove explicit displaysChanged signal, use auto-generated from displays property
- ad40d2b fix: remove duplicate Component.onCompleted and problematic Connections block
- 20a4c91 fix: defer SettingsModel access in AndroidAutoStatus to Component.onCompleted
- a7d0771 chore: add qmldir to .gitignore (auto-generated by Qt6 CMake)
- a8f3909 fix: remove conflicting primaryDisplayChanged signal, use auto-generated primaryDisplayIdChanged
- 15a5c06 fix: remove manual qmldir, let Qt 6 CMake auto-generate it
- a62b06a fix: add DisplayModel to qmldir as singleton
- 502e7e1 fix: add SettingsModel.cpp to CMakeLists and remove QML singleton marker
- d6477a3 fix: remove extra brace in SettingsModel.qml
- 27166fa fix: option D - create SettingsModel and SettingsRegistry as C++ context properties
- ad36b38 fix: add QML_ELEMENT to SettingsRegistry and delay SettingsModel init
- 9fe8305 fix: register SettingsRegistry as context property instead of QML singleton
- fa379c5 fix: register SettingsRegistry C++ singleton in qmldir
- 791ab51 fix: add SettingsRegistry include to UI main.cpp
- 65a7625 Docs: update CHANGELOG.md for 2026.01.10+git.fa44d28
- fa44d28 feat: Modern Responsive UI (AndroidAuto + Settings) (#22)
- 3a33b31 fix home
- 1f09776 Added prerm fix to apt
- 2b52e6b fix: Set default profile to use real devices (useMock=false)
- f38bcef fix: Create profiles directory in postinst
- 55719e3 format fix
- e626fd8 fix: Register AndroidAutoStatus as QML singleton in CMakeLists.txt
- e0ad389 Docs: update CHANGELOG.md for 2026.01.08+git.42b524b
- 42b524b fix: Normalise packaging scripts to LF and resolve QML singleton issues (#21)
- aaa4b6b Docs: update CHANGELOG.md for 2026.01.08+git.4158b14
- 4158b14 Bugfix/quality (#20)
- 5407ebe Docs: update CHANGELOG.md for 2026.01.07+git.2fc4b74
- 2fc4b74 format fix (#19)
- ed627bd Docs: update CHANGELOG.md for 2026.01.06+git.7800506
- 7800506 001 rpi image (#18)
- c74aa59 fix(image-builder): Make dpkg-reconfigure qemu-user-static non-interactive
- 3188f81 Docs: update CHANGELOG.md for 2026.01.05+git.3801f10
- 3801f10 fix(ci): Use trigger-apt-publish workflow instead of direct apt-publish (#17)
- 2445f76 Docs: update CHANGELOG.md for 2026.01.05+git.6408b90
- 6408b90 feat: Comprehensive GitHub Actions CI/CD Pipeline (Feature 003) (#16)
- f71dea3 Docs: update CHANGELOG.md for 2026.01.03+git.54c40bc
- 54c40bc feat: Crankshaft MVP - Phase 8 Complete (100% Project Delivery) (#15)
- 72d1dc0 Docs: update CHANGELOG.md for 2026.01.03+git.2dac628
- 2dac628 Docs: Add project constitution and enforce checks in templates (#14)
- 47e8b0d Docs: update CHANGELOG.md for 2025.12.30+git.3ed944a
- 3ed944a Added AA status widget (#13)
- 62899b9 Docs: update CHANGELOG.md for 2025.12.29+git.7bf85a9
- 7bf85a9 Fix: ProfileManager initialisation and core→UI state synchronisation (#12)
- 47d095e Docs: update CHANGELOG.md for 2025.12.21+git.0bc0d7c
- 0bc0d7c chore: Update aasdk submodule to main (feature/AA_part2) (#11)
- 734b794 Docs: update CHANGELOG.md for 2025.12.21+git.61f26ae
- 61f26ae Fix: Android Auto USB connection - wire USBHub device handle and poll io_service (#10)
- f139ccf Docs: update CHANGELOG.md for 2025.12.20+git.72246c8
- 72246c8 Feature: ProfileManager - schemas, validation, docs and build fixes (#9)
- 722b241 Docs: update CHANGELOG.md for 2025.12.19+git.5b9a14e
- 5b9a14e Fix: add qt6-connectivity-dev to Docker for Qt6 Bluetooth support (#8)
- 7421f07 Fix: remove ServiceManager stub causing vtable linkage errors (#7)
- dee56d6 Docs: update CHANGELOG.md for 2025.12.18+git.8ac2251
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2026.01.12+git.4504e9e] - 2026-01-12
- 4504e9e Slim AndroidAuto UI: add slim UI app, packaging, docs, and CI coverage (#23)
- 8c415f1 fix: comment out invalid wsConnections Connections property block
- 2598707 fix: remove stateChanged signal that conflicts with auto-generated state property signal
- a7afa71 fix: remove explicit displaysChanged signal, use auto-generated from displays property
- ad40d2b fix: remove duplicate Component.onCompleted and problematic Connections block
- 20a4c91 fix: defer SettingsModel access in AndroidAutoStatus to Component.onCompleted
- a7d0771 chore: add qmldir to .gitignore (auto-generated by Qt6 CMake)
- a8f3909 fix: remove conflicting primaryDisplayChanged signal, use auto-generated primaryDisplayIdChanged
- 15a5c06 fix: remove manual qmldir, let Qt 6 CMake auto-generate it
- a62b06a fix: add DisplayModel to qmldir as singleton
- 502e7e1 fix: add SettingsModel.cpp to CMakeLists and remove QML singleton marker
- d6477a3 fix: remove extra brace in SettingsModel.qml
- 27166fa fix: option D - create SettingsModel and SettingsRegistry as C++ context properties
- ad36b38 fix: add QML_ELEMENT to SettingsRegistry and delay SettingsModel init
- 9fe8305 fix: register SettingsRegistry as context property instead of QML singleton
- fa379c5 fix: register SettingsRegistry C++ singleton in qmldir
- 791ab51 fix: add SettingsRegistry include to UI main.cpp
- 65a7625 Docs: update CHANGELOG.md for 2026.01.10+git.fa44d28
- fa44d28 feat: Modern Responsive UI (AndroidAuto + Settings) (#22)
- 3a33b31 fix home
- 1f09776 Added prerm fix to apt
- 2b52e6b fix: Set default profile to use real devices (useMock=false)
- f38bcef fix: Create profiles directory in postinst
- 55719e3 format fix
- e626fd8 fix: Register AndroidAutoStatus as QML singleton in CMakeLists.txt
- e0ad389 Docs: update CHANGELOG.md for 2026.01.08+git.42b524b
- 42b524b fix: Normalise packaging scripts to LF and resolve QML singleton issues (#21)
- aaa4b6b Docs: update CHANGELOG.md for 2026.01.08+git.4158b14
- 4158b14 Bugfix/quality (#20)
- 5407ebe Docs: update CHANGELOG.md for 2026.01.07+git.2fc4b74
- 2fc4b74 format fix (#19)
- ed627bd Docs: update CHANGELOG.md for 2026.01.06+git.7800506
- 7800506 001 rpi image (#18)
- c74aa59 fix(image-builder): Make dpkg-reconfigure qemu-user-static non-interactive
- 3188f81 Docs: update CHANGELOG.md for 2026.01.05+git.3801f10
- 3801f10 fix(ci): Use trigger-apt-publish workflow instead of direct apt-publish (#17)
- 2445f76 Docs: update CHANGELOG.md for 2026.01.05+git.6408b90
- 6408b90 feat: Comprehensive GitHub Actions CI/CD Pipeline (Feature 003) (#16)
- f71dea3 Docs: update CHANGELOG.md for 2026.01.03+git.54c40bc
- 54c40bc feat: Crankshaft MVP - Phase 8 Complete (100% Project Delivery) (#15)
- 72d1dc0 Docs: update CHANGELOG.md for 2026.01.03+git.2dac628
- 2dac628 Docs: Add project constitution and enforce checks in templates (#14)
- 47e8b0d Docs: update CHANGELOG.md for 2025.12.30+git.3ed944a
- 3ed944a Added AA status widget (#13)
- 62899b9 Docs: update CHANGELOG.md for 2025.12.29+git.7bf85a9
- 7bf85a9 Fix: ProfileManager initialisation and core→UI state synchronisation (#12)
- 47d095e Docs: update CHANGELOG.md for 2025.12.21+git.0bc0d7c
- 0bc0d7c chore: Update aasdk submodule to main (feature/AA_part2) (#11)
- 734b794 Docs: update CHANGELOG.md for 2025.12.21+git.61f26ae
- 61f26ae Fix: Android Auto USB connection - wire USBHub device handle and poll io_service (#10)
- f139ccf Docs: update CHANGELOG.md for 2025.12.20+git.72246c8
- 72246c8 Feature: ProfileManager - schemas, validation, docs and build fixes (#9)
- 722b241 Docs: update CHANGELOG.md for 2025.12.19+git.5b9a14e
- 5b9a14e Fix: add qt6-connectivity-dev to Docker for Qt6 Bluetooth support (#8)
- 7421f07 Fix: remove ServiceManager stub causing vtable linkage errors (#7)
- dee56d6 Docs: update CHANGELOG.md for 2025.12.18+git.8ac2251
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2026.01.10+git.fa44d28] - 2026-01-10
- fa44d28 feat: Modern Responsive UI (AndroidAuto + Settings) (#22)
- 3a33b31 fix home
- 1f09776 Added prerm fix to apt
- 2b52e6b fix: Set default profile to use real devices (useMock=false)
- f38bcef fix: Create profiles directory in postinst
- 55719e3 format fix
- e626fd8 fix: Register AndroidAutoStatus as QML singleton in CMakeLists.txt
- e0ad389 Docs: update CHANGELOG.md for 2026.01.08+git.42b524b
- 42b524b fix: Normalise packaging scripts to LF and resolve QML singleton issues (#21)
- aaa4b6b Docs: update CHANGELOG.md for 2026.01.08+git.4158b14
- 4158b14 Bugfix/quality (#20)
- 5407ebe Docs: update CHANGELOG.md for 2026.01.07+git.2fc4b74
- 2fc4b74 format fix (#19)
- ed627bd Docs: update CHANGELOG.md for 2026.01.06+git.7800506
- 7800506 001 rpi image (#18)
- c74aa59 fix(image-builder): Make dpkg-reconfigure qemu-user-static non-interactive
- 3188f81 Docs: update CHANGELOG.md for 2026.01.05+git.3801f10
- 3801f10 fix(ci): Use trigger-apt-publish workflow instead of direct apt-publish (#17)
- 2445f76 Docs: update CHANGELOG.md for 2026.01.05+git.6408b90
- 6408b90 feat: Comprehensive GitHub Actions CI/CD Pipeline (Feature 003) (#16)
- f71dea3 Docs: update CHANGELOG.md for 2026.01.03+git.54c40bc
- 54c40bc feat: Crankshaft MVP - Phase 8 Complete (100% Project Delivery) (#15)
- 72d1dc0 Docs: update CHANGELOG.md for 2026.01.03+git.2dac628
- 2dac628 Docs: Add project constitution and enforce checks in templates (#14)
- 47e8b0d Docs: update CHANGELOG.md for 2025.12.30+git.3ed944a
- 3ed944a Added AA status widget (#13)
- 62899b9 Docs: update CHANGELOG.md for 2025.12.29+git.7bf85a9
- 7bf85a9 Fix: ProfileManager initialisation and core→UI state synchronisation (#12)
- 47d095e Docs: update CHANGELOG.md for 2025.12.21+git.0bc0d7c
- 0bc0d7c chore: Update aasdk submodule to main (feature/AA_part2) (#11)
- 734b794 Docs: update CHANGELOG.md for 2025.12.21+git.61f26ae
- 61f26ae Fix: Android Auto USB connection - wire USBHub device handle and poll io_service (#10)
- f139ccf Docs: update CHANGELOG.md for 2025.12.20+git.72246c8
- 72246c8 Feature: ProfileManager - schemas, validation, docs and build fixes (#9)
- 722b241 Docs: update CHANGELOG.md for 2025.12.19+git.5b9a14e
- 5b9a14e Fix: add qt6-connectivity-dev to Docker for Qt6 Bluetooth support (#8)
- 7421f07 Fix: remove ServiceManager stub causing vtable linkage errors (#7)
- dee56d6 Docs: update CHANGELOG.md for 2025.12.18+git.8ac2251
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2026.01.08+git.42b524b] - 2026-01-08
- 42b524b fix: Normalise packaging scripts to LF and resolve QML singleton issues (#21)
- aaa4b6b Docs: update CHANGELOG.md for 2026.01.08+git.4158b14
- 4158b14 Bugfix/quality (#20)
- 5407ebe Docs: update CHANGELOG.md for 2026.01.07+git.2fc4b74
- 2fc4b74 format fix (#19)
- ed627bd Docs: update CHANGELOG.md for 2026.01.06+git.7800506
- 7800506 001 rpi image (#18)
- c74aa59 fix(image-builder): Make dpkg-reconfigure qemu-user-static non-interactive
- 3188f81 Docs: update CHANGELOG.md for 2026.01.05+git.3801f10
- 3801f10 fix(ci): Use trigger-apt-publish workflow instead of direct apt-publish (#17)
- 2445f76 Docs: update CHANGELOG.md for 2026.01.05+git.6408b90
- 6408b90 feat: Comprehensive GitHub Actions CI/CD Pipeline (Feature 003) (#16)
- f71dea3 Docs: update CHANGELOG.md for 2026.01.03+git.54c40bc
- 54c40bc feat: Crankshaft MVP - Phase 8 Complete (100% Project Delivery) (#15)
- 72d1dc0 Docs: update CHANGELOG.md for 2026.01.03+git.2dac628
- 2dac628 Docs: Add project constitution and enforce checks in templates (#14)
- 47e8b0d Docs: update CHANGELOG.md for 2025.12.30+git.3ed944a
- 3ed944a Added AA status widget (#13)
- 62899b9 Docs: update CHANGELOG.md for 2025.12.29+git.7bf85a9
- 7bf85a9 Fix: ProfileManager initialisation and core→UI state synchronisation (#12)
- 47d095e Docs: update CHANGELOG.md for 2025.12.21+git.0bc0d7c
- 0bc0d7c chore: Update aasdk submodule to main (feature/AA_part2) (#11)
- 734b794 Docs: update CHANGELOG.md for 2025.12.21+git.61f26ae
- 61f26ae Fix: Android Auto USB connection - wire USBHub device handle and poll io_service (#10)
- f139ccf Docs: update CHANGELOG.md for 2025.12.20+git.72246c8
- 72246c8 Feature: ProfileManager - schemas, validation, docs and build fixes (#9)
- 722b241 Docs: update CHANGELOG.md for 2025.12.19+git.5b9a14e
- 5b9a14e Fix: add qt6-connectivity-dev to Docker for Qt6 Bluetooth support (#8)
- 7421f07 Fix: remove ServiceManager stub causing vtable linkage errors (#7)
- dee56d6 Docs: update CHANGELOG.md for 2025.12.18+git.8ac2251
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2026.01.08+git.4158b14] - 2026-01-08
- 4158b14 Bugfix/quality (#20)
- 5407ebe Docs: update CHANGELOG.md for 2026.01.07+git.2fc4b74
- 2fc4b74 format fix (#19)
- ed627bd Docs: update CHANGELOG.md for 2026.01.06+git.7800506
- 7800506 001 rpi image (#18)
- c74aa59 fix(image-builder): Make dpkg-reconfigure qemu-user-static non-interactive
- 3188f81 Docs: update CHANGELOG.md for 2026.01.05+git.3801f10
- 3801f10 fix(ci): Use trigger-apt-publish workflow instead of direct apt-publish (#17)
- 2445f76 Docs: update CHANGELOG.md for 2026.01.05+git.6408b90
- 6408b90 feat: Comprehensive GitHub Actions CI/CD Pipeline (Feature 003) (#16)
- f71dea3 Docs: update CHANGELOG.md for 2026.01.03+git.54c40bc
- 54c40bc feat: Crankshaft MVP - Phase 8 Complete (100% Project Delivery) (#15)
- 72d1dc0 Docs: update CHANGELOG.md for 2026.01.03+git.2dac628
- 2dac628 Docs: Add project constitution and enforce checks in templates (#14)
- 47e8b0d Docs: update CHANGELOG.md for 2025.12.30+git.3ed944a
- 3ed944a Added AA status widget (#13)
- 62899b9 Docs: update CHANGELOG.md for 2025.12.29+git.7bf85a9
- 7bf85a9 Fix: ProfileManager initialisation and core→UI state synchronisation (#12)
- 47d095e Docs: update CHANGELOG.md for 2025.12.21+git.0bc0d7c
- 0bc0d7c chore: Update aasdk submodule to main (feature/AA_part2) (#11)
- 734b794 Docs: update CHANGELOG.md for 2025.12.21+git.61f26ae
- 61f26ae Fix: Android Auto USB connection - wire USBHub device handle and poll io_service (#10)
- f139ccf Docs: update CHANGELOG.md for 2025.12.20+git.72246c8
- 72246c8 Feature: ProfileManager - schemas, validation, docs and build fixes (#9)
- 722b241 Docs: update CHANGELOG.md for 2025.12.19+git.5b9a14e
- 5b9a14e Fix: add qt6-connectivity-dev to Docker for Qt6 Bluetooth support (#8)
- 7421f07 Fix: remove ServiceManager stub causing vtable linkage errors (#7)
- dee56d6 Docs: update CHANGELOG.md for 2025.12.18+git.8ac2251
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2026.01.07+git.2fc4b74] - 2026-01-07
- 2fc4b74 format fix (#19)
- ed627bd Docs: update CHANGELOG.md for 2026.01.06+git.7800506
- 7800506 001 rpi image (#18)
- c74aa59 fix(image-builder): Make dpkg-reconfigure qemu-user-static non-interactive
- 3188f81 Docs: update CHANGELOG.md for 2026.01.05+git.3801f10
- 3801f10 fix(ci): Use trigger-apt-publish workflow instead of direct apt-publish (#17)
- 2445f76 Docs: update CHANGELOG.md for 2026.01.05+git.6408b90
- 6408b90 feat: Comprehensive GitHub Actions CI/CD Pipeline (Feature 003) (#16)
- f71dea3 Docs: update CHANGELOG.md for 2026.01.03+git.54c40bc
- 54c40bc feat: Crankshaft MVP - Phase 8 Complete (100% Project Delivery) (#15)
- 72d1dc0 Docs: update CHANGELOG.md for 2026.01.03+git.2dac628
- 2dac628 Docs: Add project constitution and enforce checks in templates (#14)
- 47e8b0d Docs: update CHANGELOG.md for 2025.12.30+git.3ed944a
- 3ed944a Added AA status widget (#13)
- 62899b9 Docs: update CHANGELOG.md for 2025.12.29+git.7bf85a9
- 7bf85a9 Fix: ProfileManager initialisation and core→UI state synchronisation (#12)
- 47d095e Docs: update CHANGELOG.md for 2025.12.21+git.0bc0d7c
- 0bc0d7c chore: Update aasdk submodule to main (feature/AA_part2) (#11)
- 734b794 Docs: update CHANGELOG.md for 2025.12.21+git.61f26ae
- 61f26ae Fix: Android Auto USB connection - wire USBHub device handle and poll io_service (#10)
- f139ccf Docs: update CHANGELOG.md for 2025.12.20+git.72246c8
- 72246c8 Feature: ProfileManager - schemas, validation, docs and build fixes (#9)
- 722b241 Docs: update CHANGELOG.md for 2025.12.19+git.5b9a14e
- 5b9a14e Fix: add qt6-connectivity-dev to Docker for Qt6 Bluetooth support (#8)
- 7421f07 Fix: remove ServiceManager stub causing vtable linkage errors (#7)
- dee56d6 Docs: update CHANGELOG.md for 2025.12.18+git.8ac2251
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2026.01.06+git.7800506] - 2026-01-06
- 7800506 001 rpi image (#18)
- c74aa59 fix(image-builder): Make dpkg-reconfigure qemu-user-static non-interactive
- 3188f81 Docs: update CHANGELOG.md for 2026.01.05+git.3801f10
- 3801f10 fix(ci): Use trigger-apt-publish workflow instead of direct apt-publish (#17)
- 2445f76 Docs: update CHANGELOG.md for 2026.01.05+git.6408b90
- 6408b90 feat: Comprehensive GitHub Actions CI/CD Pipeline (Feature 003) (#16)
- f71dea3 Docs: update CHANGELOG.md for 2026.01.03+git.54c40bc
- 54c40bc feat: Crankshaft MVP - Phase 8 Complete (100% Project Delivery) (#15)
- 72d1dc0 Docs: update CHANGELOG.md for 2026.01.03+git.2dac628
- 2dac628 Docs: Add project constitution and enforce checks in templates (#14)
- 47e8b0d Docs: update CHANGELOG.md for 2025.12.30+git.3ed944a
- 3ed944a Added AA status widget (#13)
- 62899b9 Docs: update CHANGELOG.md for 2025.12.29+git.7bf85a9
- 7bf85a9 Fix: ProfileManager initialisation and core→UI state synchronisation (#12)
- 47d095e Docs: update CHANGELOG.md for 2025.12.21+git.0bc0d7c
- 0bc0d7c chore: Update aasdk submodule to main (feature/AA_part2) (#11)
- 734b794 Docs: update CHANGELOG.md for 2025.12.21+git.61f26ae
- 61f26ae Fix: Android Auto USB connection - wire USBHub device handle and poll io_service (#10)
- f139ccf Docs: update CHANGELOG.md for 2025.12.20+git.72246c8
- 72246c8 Feature: ProfileManager - schemas, validation, docs and build fixes (#9)
- 722b241 Docs: update CHANGELOG.md for 2025.12.19+git.5b9a14e
- 5b9a14e Fix: add qt6-connectivity-dev to Docker for Qt6 Bluetooth support (#8)
- 7421f07 Fix: remove ServiceManager stub causing vtable linkage errors (#7)
- dee56d6 Docs: update CHANGELOG.md for 2025.12.18+git.8ac2251
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2026.01.05+git.3801f10] - 2026-01-05
- 3801f10 fix(ci): Use trigger-apt-publish workflow instead of direct apt-publish (#17)
- 2445f76 Docs: update CHANGELOG.md for 2026.01.05+git.6408b90
- 6408b90 feat: Comprehensive GitHub Actions CI/CD Pipeline (Feature 003) (#16)
- f71dea3 Docs: update CHANGELOG.md for 2026.01.03+git.54c40bc
- 54c40bc feat: Crankshaft MVP - Phase 8 Complete (100% Project Delivery) (#15)
- 72d1dc0 Docs: update CHANGELOG.md for 2026.01.03+git.2dac628
- 2dac628 Docs: Add project constitution and enforce checks in templates (#14)
- 47e8b0d Docs: update CHANGELOG.md for 2025.12.30+git.3ed944a
- 3ed944a Added AA status widget (#13)
- 62899b9 Docs: update CHANGELOG.md for 2025.12.29+git.7bf85a9
- 7bf85a9 Fix: ProfileManager initialisation and core→UI state synchronisation (#12)
- 47d095e Docs: update CHANGELOG.md for 2025.12.21+git.0bc0d7c
- 0bc0d7c chore: Update aasdk submodule to main (feature/AA_part2) (#11)
- 734b794 Docs: update CHANGELOG.md for 2025.12.21+git.61f26ae
- 61f26ae Fix: Android Auto USB connection - wire USBHub device handle and poll io_service (#10)
- f139ccf Docs: update CHANGELOG.md for 2025.12.20+git.72246c8
- 72246c8 Feature: ProfileManager - schemas, validation, docs and build fixes (#9)
- 722b241 Docs: update CHANGELOG.md for 2025.12.19+git.5b9a14e
- 5b9a14e Fix: add qt6-connectivity-dev to Docker for Qt6 Bluetooth support (#8)
- 7421f07 Fix: remove ServiceManager stub causing vtable linkage errors (#7)
- dee56d6 Docs: update CHANGELOG.md for 2025.12.18+git.8ac2251
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2026.01.05+git.6408b90] - 2026-01-05
- 6408b90 feat: Comprehensive GitHub Actions CI/CD Pipeline (Feature 003) (#16)
- f71dea3 Docs: update CHANGELOG.md for 2026.01.03+git.54c40bc
- 54c40bc feat: Crankshaft MVP - Phase 8 Complete (100% Project Delivery) (#15)
- 72d1dc0 Docs: update CHANGELOG.md for 2026.01.03+git.2dac628
- 2dac628 Docs: Add project constitution and enforce checks in templates (#14)
- 47e8b0d Docs: update CHANGELOG.md for 2025.12.30+git.3ed944a
- 3ed944a Added AA status widget (#13)
- 62899b9 Docs: update CHANGELOG.md for 2025.12.29+git.7bf85a9
- 7bf85a9 Fix: ProfileManager initialisation and core→UI state synchronisation (#12)
- 47d095e Docs: update CHANGELOG.md for 2025.12.21+git.0bc0d7c
- 0bc0d7c chore: Update aasdk submodule to main (feature/AA_part2) (#11)
- 734b794 Docs: update CHANGELOG.md for 2025.12.21+git.61f26ae
- 61f26ae Fix: Android Auto USB connection - wire USBHub device handle and poll io_service (#10)
- f139ccf Docs: update CHANGELOG.md for 2025.12.20+git.72246c8
- 72246c8 Feature: ProfileManager - schemas, validation, docs and build fixes (#9)
- 722b241 Docs: update CHANGELOG.md for 2025.12.19+git.5b9a14e
- 5b9a14e Fix: add qt6-connectivity-dev to Docker for Qt6 Bluetooth support (#8)
- 7421f07 Fix: remove ServiceManager stub causing vtable linkage errors (#7)
- dee56d6 Docs: update CHANGELOG.md for 2025.12.18+git.8ac2251
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2026.01.03+git.54c40bc] - 2026-01-03
- 54c40bc feat: Crankshaft MVP - Phase 8 Complete (100% Project Delivery) (#15)
- 72d1dc0 Docs: update CHANGELOG.md for 2026.01.03+git.2dac628
- 2dac628 Docs: Add project constitution and enforce checks in templates (#14)
- 47e8b0d Docs: update CHANGELOG.md for 2025.12.30+git.3ed944a
- 3ed944a Added AA status widget (#13)
- 62899b9 Docs: update CHANGELOG.md for 2025.12.29+git.7bf85a9
- 7bf85a9 Fix: ProfileManager initialisation and core→UI state synchronisation (#12)
- 47d095e Docs: update CHANGELOG.md for 2025.12.21+git.0bc0d7c
- 0bc0d7c chore: Update aasdk submodule to main (feature/AA_part2) (#11)
- 734b794 Docs: update CHANGELOG.md for 2025.12.21+git.61f26ae
- 61f26ae Fix: Android Auto USB connection - wire USBHub device handle and poll io_service (#10)
- f139ccf Docs: update CHANGELOG.md for 2025.12.20+git.72246c8
- 72246c8 Feature: ProfileManager - schemas, validation, docs and build fixes (#9)
- 722b241 Docs: update CHANGELOG.md for 2025.12.19+git.5b9a14e
- 5b9a14e Fix: add qt6-connectivity-dev to Docker for Qt6 Bluetooth support (#8)
- 7421f07 Fix: remove ServiceManager stub causing vtable linkage errors (#7)
- dee56d6 Docs: update CHANGELOG.md for 2025.12.18+git.8ac2251
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2026.01.03+git.2dac628] - 2026-01-03
- 2dac628 Docs: Add project constitution and enforce checks in templates (#14)
- 47e8b0d Docs: update CHANGELOG.md for 2025.12.30+git.3ed944a
- 3ed944a Added AA status widget (#13)
- 62899b9 Docs: update CHANGELOG.md for 2025.12.29+git.7bf85a9
- 7bf85a9 Fix: ProfileManager initialisation and core→UI state synchronisation (#12)
- 47d095e Docs: update CHANGELOG.md for 2025.12.21+git.0bc0d7c
- 0bc0d7c chore: Update aasdk submodule to main (feature/AA_part2) (#11)
- 734b794 Docs: update CHANGELOG.md for 2025.12.21+git.61f26ae
- 61f26ae Fix: Android Auto USB connection - wire USBHub device handle and poll io_service (#10)
- f139ccf Docs: update CHANGELOG.md for 2025.12.20+git.72246c8
- 72246c8 Feature: ProfileManager - schemas, validation, docs and build fixes (#9)
- 722b241 Docs: update CHANGELOG.md for 2025.12.19+git.5b9a14e
- 5b9a14e Fix: add qt6-connectivity-dev to Docker for Qt6 Bluetooth support (#8)
- 7421f07 Fix: remove ServiceManager stub causing vtable linkage errors (#7)
- dee56d6 Docs: update CHANGELOG.md for 2025.12.18+git.8ac2251
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2025.12.30+git.3ed944a] - 2025-12-30
- 3ed944a Added AA status widget (#13)
- 62899b9 Docs: update CHANGELOG.md for 2025.12.29+git.7bf85a9
- 7bf85a9 Fix: ProfileManager initialisation and core→UI state synchronisation (#12)
- 47d095e Docs: update CHANGELOG.md for 2025.12.21+git.0bc0d7c
- 0bc0d7c chore: Update aasdk submodule to main (feature/AA_part2) (#11)
- 734b794 Docs: update CHANGELOG.md for 2025.12.21+git.61f26ae
- 61f26ae Fix: Android Auto USB connection - wire USBHub device handle and poll io_service (#10)
- f139ccf Docs: update CHANGELOG.md for 2025.12.20+git.72246c8
- 72246c8 Feature: ProfileManager - schemas, validation, docs and build fixes (#9)
- 722b241 Docs: update CHANGELOG.md for 2025.12.19+git.5b9a14e
- 5b9a14e Fix: add qt6-connectivity-dev to Docker for Qt6 Bluetooth support (#8)
- 7421f07 Fix: remove ServiceManager stub causing vtable linkage errors (#7)
- dee56d6 Docs: update CHANGELOG.md for 2025.12.18+git.8ac2251
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2025.12.29+git.7bf85a9] - 2025-12-29
- 7bf85a9 Fix: ProfileManager initialisation and core→UI state synchronisation (#12)
- 47d095e Docs: update CHANGELOG.md for 2025.12.21+git.0bc0d7c
- 0bc0d7c chore: Update aasdk submodule to main (feature/AA_part2) (#11)
- 734b794 Docs: update CHANGELOG.md for 2025.12.21+git.61f26ae
- 61f26ae Fix: Android Auto USB connection - wire USBHub device handle and poll io_service (#10)
- f139ccf Docs: update CHANGELOG.md for 2025.12.20+git.72246c8
- 72246c8 Feature: ProfileManager - schemas, validation, docs and build fixes (#9)
- 722b241 Docs: update CHANGELOG.md for 2025.12.19+git.5b9a14e
- 5b9a14e Fix: add qt6-connectivity-dev to Docker for Qt6 Bluetooth support (#8)
- 7421f07 Fix: remove ServiceManager stub causing vtable linkage errors (#7)
- dee56d6 Docs: update CHANGELOG.md for 2025.12.18+git.8ac2251
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2025.12.21+git.0bc0d7c] - 2025-12-21
- 0bc0d7c chore: Update aasdk submodule to main (feature/AA_part2) (#11)
- 734b794 Docs: update CHANGELOG.md for 2025.12.21+git.61f26ae
- 61f26ae Fix: Android Auto USB connection - wire USBHub device handle and poll io_service (#10)
- f139ccf Docs: update CHANGELOG.md for 2025.12.20+git.72246c8
- 72246c8 Feature: ProfileManager - schemas, validation, docs and build fixes (#9)
- 722b241 Docs: update CHANGELOG.md for 2025.12.19+git.5b9a14e
- 5b9a14e Fix: add qt6-connectivity-dev to Docker for Qt6 Bluetooth support (#8)
- 7421f07 Fix: remove ServiceManager stub causing vtable linkage errors (#7)
- dee56d6 Docs: update CHANGELOG.md for 2025.12.18+git.8ac2251
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2025.12.21+git.61f26ae] - 2025-12-21
- 61f26ae Fix: Android Auto USB connection - wire USBHub device handle and poll io_service (#10)
- f139ccf Docs: update CHANGELOG.md for 2025.12.20+git.72246c8
- 72246c8 Feature: ProfileManager - schemas, validation, docs and build fixes (#9)
- 722b241 Docs: update CHANGELOG.md for 2025.12.19+git.5b9a14e
- 5b9a14e Fix: add qt6-connectivity-dev to Docker for Qt6 Bluetooth support (#8)
- 7421f07 Fix: remove ServiceManager stub causing vtable linkage errors (#7)
- dee56d6 Docs: update CHANGELOG.md for 2025.12.18+git.8ac2251
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2025.12.20+git.72246c8] - 2025-12-20
- 72246c8 Feature: ProfileManager - schemas, validation, docs and build fixes (#9)
- 722b241 Docs: update CHANGELOG.md for 2025.12.19+git.5b9a14e
- 5b9a14e Fix: add qt6-connectivity-dev to Docker for Qt6 Bluetooth support (#8)
- 7421f07 Fix: remove ServiceManager stub causing vtable linkage errors (#7)
- dee56d6 Docs: update CHANGELOG.md for 2025.12.18+git.8ac2251
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2025.12.19+git.5b9a14e] - 2025-12-19
- 5b9a14e Fix: add qt6-connectivity-dev to Docker for Qt6 Bluetooth support (#8)
- 7421f07 Fix: remove ServiceManager stub causing vtable linkage errors (#7)
- dee56d6 Docs: update CHANGELOG.md for 2025.12.18+git.8ac2251
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2025.12.18+git.8ac2251] - 2025-12-18
- 8ac2251 fix psh
- f3f0d25 Fix: decouple WebSocketServer test from ServiceManager deps

- Replace linking of ServiceManager.cpp with test stub to avoid Qt Bluetooth dependency
- Add tests/stubs/ServiceManager_stub.cpp implementing minimal ServiceManager methods
- Keeps unit tests focused and buildable in CI (no Qt Bluetooth)
- 41e3c3c Added export log
- 6f36daa Docs: update CHANGELOG.md for 2025.12.18+git.1572d82
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## [2025.12.18+git.1572d82] - 2025-12-18
- 1572d82 Fix: add ServiceManager.cpp to test_websocket linking
- 5441759 removed temp logs
- 5731125 Fix: add missing QJsonArray include and build validation
- 88518f3 [skip ci] - changelog
- 3b5253e Add: Initial Android Auto support with wireless connectivity and service orchestration (#3)
- f19efe0 update user rename to disable
- 3990832 [skip ci]
- 98f8048 Added fix for utf-8 added pipeline fix
- da90348 added depends manually
- a84bcba Fix: remove non-existent qt6-base-platforms dependency from crankshaft-ui package; platform plugins are built-in to libqt6gui6/libqt6core6
- 72eec04 Fix: add actions:write permission to dispatch-cd-fastpath job
- bec2126 Chore: CI dispatches CD with amd64only=true on manual fast-path runs
- 65dc5cc Fix: CD test job sets up checkout + Buildx and uses explicit Dockerfile path; respect amd64only on manual dispatch
- 6c5b808 add amd64 only
- c600c21 fixed depends, split out apt test to file.
- d0fce4b fix(packaging): add alternative deps for trixie (t64 | non-t64) to improve ARM install resilience
- 55c0808 fix(packaging): use libssl3t64 for trixie, libqt6core6t64/libqt6gui6t64 for all archs in trixie
- 8c7c057 ci/cd: re-enable arm64/armhf builds across all workflows
- 4b61925 ci: build amd64 only (temporary)
- 7ed2188 ci/cd: disable arm64/armhf in CD + PiGen; guard systemctl in postinst for container installs
- d22df6e updated ci filter
- fe5624e Added systemd guards
- e81fc82 chore: add Debian suite defaults for builds
- 46ca088 added check for publish completed successfully
- a0f2a67 fixed typo
- d29e09f added apt test before pi-gen
- bd8794d added service, udev and usb action
- d14366b fixed depends
- 2f658c8 updated depends
- ad1aa67 repoint apt
- 253a780 update workflow to wait for apt
- ce8d395 enabled deps default
- bac1d9a delete backup files
- 52d349c switched to aasdk main
- 6544fd6 added aasdk runtime
- f936e76 Added publish src package
- 2052556 fix logic to only trigger on successes
- 1700ea1 fixed regex
- 0f5bf81 update version handling
- da3b253 Add condition to onyl trigger cd on success
- 840e375 updated version handling
- 3993508 fixed triggers
- c418325 refactor to use Cpack to build src package
- 37f2ce8 Removed triggers
- 9250003 refactored deb creation
- 91053f9 fix lookup
- 6732f03 updated dockerfile with lsb
- b4f736d updated dockerfile with lsb
- e2c35f1 updated dockerfile with lsb
- 094b9f2 Add  debfilegen
- 831126f [skip ci]
- 3e80ba9 [skip ci]
- b0fda02 [skip ci]
- 6909590 Feature: CD auto-resolves latest successful CI run when build_run_id omitted; use workflow IDs for dispatch
- fe85124 [skip ci]
- d29d18f Fix: CD dispatch error (ENOENT) by removing fs.readFileSync; use context payload for workflow_run/workflow_dispatch
- 56c186b [skip ci]
- 96714d1 Feature: Skip CI based on commit message
- 944c7ea Feature: Add path filters to CI workflow for faster builds
- cd09996 Feature: Add workflow_dispatch to CD, build-pi-gen-lite and changelog workflows for manual testing
- ece0e5c Fix: CD workflow startup failure - use workflow_dispatch instead of workflow_call
- 5566951 disable broken
- d815ca3 Fix source tar to archive from build dir and exclude output dirs
- 3485217 Fix source deb tar to exclude build artifacts and output directory
- 8b5d588  CRLF to LF
- 1899a03 clang format
- 807d0a0 Add code quality lint script and workflow documentation
- 99f1a7d Fix source deb RUN continuation in Dockerfile
- cced3fa added workflow docs
- af5440d Refactor workflows into CI/CD/Release chain
- 9c249b0 Add source deb packaging alongside core and UI
- fd0db0f Use CPack component DEBs for core and UI packages
- 3fab441 comment out untilmain merge
- a50d9ef Added imaging workflows and gitattributes
- ee42111 fixing workflow logic
- ebca9ac Add changelog generation after successful PR builds
- c172374 Limit feature branch builds to amd64 and run changelog before build on main
- 148b960 Add build log capture and artifact upload for all architectures
- fece81c fixed docker fromas casing
- b6afc77 Fix: correct jq syntax for SBOM generation and remove non-existent artifact uploads
- 6fd1223 Fix: make SBOM timestamp JSON-safe
- b97d4f4 Fix: create output directory before building DEB package
- 35cf955 Fix: correct cmake install command for Debian packaging
- 419501f Fix: use relative paths for QML files and ensure MDI output directory exists
- 7b5d5c2 rename components tolower
- a6d4201 rename components
- 87dc074 Fix: use absolute paths for QML files in CMakeLists.txt
- fffbad8 Fix: add GStreamer and Python deps to Dockerfile
- d444683 Docs: add changelog workflow and about link
- d9003e7 Chore: update maintainer email
- 95a3fdf Feat: add acknowledgements and SBOM CI workflows
- 39a8f61 Feat: Use Material Design Icons for settings category icons
- a79a3bb Refactor: Consolidate Android Auto settings into SettingsModel
- c5305c9 Fix: Android Auto Settings UI layout and color issues
- b8d3228 Fix: QML import path for Android Auto settings page
- fd52a66 Fix: Settings UI — resolve header overlap and navigation issues
- 06c0d0e Fix: HALs — implement MediaPipeline; add WiFi/Bluetooth HAL headers; enable QtDBus in CMake
- 394cf24 Fix: Use SettingsModel singleton correctly in QML
- 41eb7f7 Standardize UI event handling - Remove lifecycle event publishing and unnecessary navigation events
- 92392ef Fix: Remove invalid onItemPushed signal handler - Use Component.onCompleted instead
- fbe36dc Fix: Navigation issues in AndroidAutoScreen and SettingsScreen - Enable proper back button functionality
- c4c453f feat: HAL implementation with multimedia, wireless, and Android Auto support
- d021448 Implemented HAL
- 0aca9a9 UI tweaks
- 855d853 MDI Icons
- b609637 Added packaging to deb
- db40ae1 fix de
- 87fe4f4 fixed translatons
- 1db6e95 modern
- 644f331 Initial MVP

## 0.1.0
- Initial MVP: Core (C++/Qt6) and UI (QML/Qt6) with WebSocket bridge and EventBus
- Theme system (light/dark), i18n (en-GB, de-DE), Material Design Icons
- Unit tests and base CI pipeline
